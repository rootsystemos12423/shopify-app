...
class Decision extends AbstractBlock
{
	...
	protected function interpretCondition($left, $right, $op, Context $context)
	{
		if (is_null($op)) {
		  ...
		  // Check for blank keyword BEFORE processing values
		  if ($right == 'blank') {
			$leftValue = $context->get($left);
			
			// Check if the value is blank (null, empty string, whitespace, or empty array)
			$isBlank = ($leftValue === null || $leftValue === '' || 
				    (is_string($leftValue) && trim($leftValue) === '') || 
				    (is_array($leftValue) && count($leftValue) === 0));
			
			return ($op == '==' || $op == 'is') ? $isBlank : !$isBlank;
		  }
