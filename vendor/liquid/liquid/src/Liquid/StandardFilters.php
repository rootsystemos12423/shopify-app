...
class StandardFilters
{
	...
	public static function divided_by($input, $operand)
    {
        try {
            if ((float)$operand == 0) {
                throw new \InvalidArgumentException("Division by zero attempted");
            }
            return (float)$input / (float)$operand;
        } catch (\Exception $e) {
		/*
            \Log::warning('Liquid Division Error', [
                'input' => $input,
                'operand' => $operand,
		...
		*/
            return 0; // Ou null, ou $input - dependendo da sua lógica
        }
    }
	...
	{
	// Se for um array, converter para string ou usar o primeiro elemento
	if (is_array($input)) {
