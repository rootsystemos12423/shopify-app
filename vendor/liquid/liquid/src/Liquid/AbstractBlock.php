...
class AbstractBlock extends AbstractTag
{
	...
	foreach ($list as $token) {
		...
		// Tratamento seguro para arrays
		if (is_array($value)) {
			// Converter array para JSON
			$value = json_encode($value, JSON_UNESCAPED_UNICODE);
		} elseif (!is_string($value) && !is_numeric($value)) {
			// Converter outros tipos não-string para string
			$value = (string)$value;
		}
	...
	{
	// Debug info - obter informações sobre o contexto atual
	$trace = debug_backtrace(DEBUG_BACKTRACE_IGNORE_ARGS, 10);
	$liquidFiles = [];
	
	foreach ($trace as $item) {
		if (isset($item['file']) && strpos($item['file'], 'liquid') !== false) {
			$liquidFiles[] = $item['file'] . ':' . $item['line'];
		}
	}
	
	// Incluir informações sobre nodelist e contexto de análise
	$nodelistInfo = '';
	if (!empty($this->nodelist)) {
		$lastNodes = array_slice($this->nodelist, -5); // últimos 5 nós para contexto
		foreach ($lastNodes as $node) {
			if (is_string($node)) {
			$nodelistInfo .= "\nString node: " . substr($node, 0, 100) . (strlen($node) > 100 ? '...' : '');
			} else {
			$nodelistInfo .= "\nObject node: " . get_class($node);
			}
		}
	}
	
	// Registrar informações adicionais de depuração
	error_log("Tag incompleta '" . $this->blockName() . "' - Contexto de análise: " . implode(' -> ', $liquidFiles));
	error_log("Últimos nós analisados: " . $nodelistInfo);
